  ...
  <div v-if="!error" class="flex flex-col justify-center items-center bg-primary">
      ...
      <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="border border-white p-8 flex rounded-2xl flex-col items-start gap-8">
          ...
          <ul class="text-white text-sm font-light flex flex-col gap-4 items-start">
            <li class="flex flex-row justify-center items-center">
              <icon name="uil:check" class="text-2xl text-green-500" /><span>Zaručíme bezproblémový beh stránky po celý
                rok</span>
            </li>
            <li class="flex flex-row justify-center items-center">
          ...
          <button @click="advPopupVisible = true"
    ...
    <Dialog v-model:visible="basePopupVisible" modal :draggable="false" header="Iba doménu a hosting"
      ...
      <div class="flex flex-col gap-8">
        ...
        <div class="flex flex-row justify-center items-center gap-4 text-sm"><Button
    ...
    <Dialog v-model:visible="advPopupVisible" modal :draggable="false" :header="plansMap[userOptions.plan]+' plán'"
      class="max-w-700p min-w-400p">
      <div class="flex flex-col gap-8">
        <div class="flex flex-row justify-start items-center gap-4 text-sm">
          ...
          <label for="periodMonth " class="ml-2">Mesačne</label>
        </div>
        <div class="flex flex-row justify-start items-center gap-4 text-sm">
        ...
        <div class="flex items-center">
          ...
          <label for="remember" class="ml-2"> Pamätať voľbu </label>
        </div>
        <div class="flex flex-row justify-center items-center gap-4 text-sm">
          ...
          <Button
            class="bg-primary border border-green-900 bg-green-700 rounded-full py-2 px-4 text-white transition-all duration-500 hover:bg-transparent hover:text-green-600">Odoslať</Button>
        </div>
      </div>
    </Dialog>
...
const plansMap = {
  basic: {name: "Základný", price:{month: "24,90 € mesačne", year: 0}},
  pro:  "Pokročilý"
}

const userOptions = ref({ plan: 'basic', period: '', remember: null })
